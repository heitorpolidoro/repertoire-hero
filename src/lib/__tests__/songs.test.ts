...
describe.skipIf(skip)('songs service integration tests', () => {
  ...
  it('createAndAddSong creates a new global song and adds to user repertoire', async () => {
    ...
    const entry = await createAndAddSong(songData)
  ...
  it('addSongToRepertoire adds an existing global song to user repertoire', async () => {
    ...
    const entry = await addSongToRepertoire(songId)
    expect(entry).toBeDefined()
  ...
  it('updateSongStatus updates the song status', async () => {
    ...
    await updateSongStatus(entry.id, 'learning')

    // Fetch again to verify
    const updated = await getSongEntry(entry.id)
  ...
  it('updatePersonalKey updates the personal key', async () => {
    ...
    await updatePersonalKey(entry.id, 'G#')
  ...
  it('removeSongFromRepertoire deletes the repertoire entry', async () => {
    ...
    const before = await getSongEntry(entry.id)
    expect(before).not.toBeNull()

    await removeSongFromRepertoire(entry.id)

    // Verify it is gone
    const after = await getSongEntry(entry.id)
    expect(after).toBeNull()
  })

  it('searchGlobalSongs searches by title or artist', async () => {
    // Add two test songs
    const song1 = await createAndAddSong({ title: `SearchTitle_${suffix}`, artist: 'SomeArtist' })
    const song2 = await createAndAddSong({ title: 'SomeTitle', artist: `SearchArtist_${suffix}` })
  ...
  it('getSongEntry retrieves a single entry or returns null if not found', async () => {
    ...
    const retrieved = await getSongEntry(entry.id)
    expect(retrieved).not.toBeNull()
    expect(retrieved!.id).toBe(entry.id)
    expect(retrieved!.song?.title).toBe(songData.title)

    // Non-existent ID
    const nonExistent = await getSongEntry('00000000-0000-0000-0000-000000000000')
  ...
  it('updateSong updates both global song details and repertoire details', async () => {
    ...
    // Call updateSong
    await updateSong(entry, updateData)
