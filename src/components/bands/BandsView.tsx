...
/**
 * The Server Actions the create form calls. Injected by `src/app/bands/page.tsx`
 * rather than imported, so `src/components` never points back into `src/app` (F21).
 */
export interface BandsViewActions {
  createBand: (
    name: string,
...
interface BandsViewProps {
  /** Read on the server by `getBands(userId)`; this island never fetches. */
  bands: Band[];
  /** A server-side read failure, surfaced in the same place as a create failure. */
  initialError: string | null;
  actions: BandsViewActions;
}
...
export function BandsView({ bands, initialError, actions }: BandsViewProps) {
  const router = useRouter();
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      ...
      const bandId = await actions.createBand(newName.trim(), newDesc.trim() || null, uploadedCoverUrl, newColor);
      // The list above is a Server Component read: drop its router cache entry
      // so a back-navigation re-runs it and shows the band just created.
      router.refresh();
