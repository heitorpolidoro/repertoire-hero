...
**Status:** Resolved by RH-66 (`882806a`), RH-67 (`5c38206`), RH-68 (`acbbf83`), RH-69 (`a79abef`), RH-70 (`bd3c8cf`) and RH-71 (the commit carrying this line, on top of `bd3c8cf`) - the six-part split RH-53 planned. `src/app/playlists/[id]/page.tsx` went from 1344 lines with complexity 34 and 24 `useState` calls to a 60-line async Server Component that resolves the session itself, reads `getPlaylistWithSongs` and `getRepertoire` through `src/lib`, and injects five Server Actions plus `SONG_PICKER_ACTIONS` into one client island. RH-67 took the dual-source add-song picker (`SongPicker`, `useSongPicker`, `src/lib/songPicker.ts`), RH-68 the song row, list and summary with the pure decisions in `src/lib/playlistDetail.ts`, RH-69 both tag bars onto the shared `useTagEditor`, RH-70 the header, the Spotify sync (`useSpotifySync`) and the four focus effects, whose panel flags became the `useReducer` over `src/lib/playlistPanels.ts` the remediation asked for, and RH-71 the remainder, as the island `src/components/playlists/PlaylistDetailView.tsx`, the controller `src/hooks/usePlaylistDetail.ts` and the pure optimistic overlay `src/lib/playlistOverlay.ts` - none of which takes a per-file override, which is what makes the split a fix rather than a relocation. The page's own `complexity-budget/override` entry is deleted outright rather than re-homed, taking the ratchet from 20 to 19 entries with `MAX_OVERRIDES` and AGENTS.md moved in lockstep. RH-66's characterization net `e2e/playlist-detail.spec.ts` kept every assertion it had through all five moves and gained only a hydration wait in RH-71, once the deleted loading state stopped providing one. The conversion also carries a user-visible delta the RH-71 spec accepts: the route now scopes its repertoire to the playlist's own owner (`const bandId = playlist.band_id`) instead of to the browsing user's active band context, and that same value is now the write owner of both repertoire writes on the route, where every write used to go out as the personal one whatever hat was on - so the band ContextSwitcher no longer affects this route at all, a song-tag edit under a band hat no longer fails with `Repertoire entry not found or access denied` (a status cycle never produced that error, because a band row renders a read-only badge and offers no button to press), and on a band playlist browsed in the personal hat a song tag now lands on the band's shared row instead of the user's own.
...
**Status:** Resolved by RH-42 (the commit carrying this line, on top of `890a27b`). `updateEmail` and `updateEmailAction` are deleted outright, not kept behind `checkSystemAdmin`: there is no admin caller today, `npm run lint:dead` fails on an export nobody imports, and a privileged raw-identity write kept alive "for later" is exactly what the new guard exists to prevent - `grep -rnw "updateEmail" src` printed 17 lines at `890a27b` and prints nothing now. The user-facing path is `requestEmailChangeAction` -> `requestEmailChange` (`src/lib/emailChange.ts`) -> `auth.api.changeEmail`, with `user.changeEmail.enabled: true`, `updateEmailWithoutVerification: false` and `emailVerification.expiresIn: 3600` configured in `src/lib/auth.ts` and asserted in `src/lib/__tests__/authConfig.test.ts`. Verification always ends at the **new** address, because reaching it is the only thing that proves control of it and moving the login onto an address the user does not own is this finding's actual risk; a user whose current address is already verified first gets a confirmation at the **current** address (`update-user.mjs` L468), which is the hijack alarm telling the address that is losing the account that something is happening, and opening it then triggers the new-address verification (`email-verification.mjs` L191-205). Both mails go through `src/lib/authEmail.ts`, the extraction of the plumbing that previously served `sendResetPassword` alone. Format, emptiness and same-as-current are validated in `validateEmailChange`; **uniqueness is deliberately not pre-checked** - a distinct "that address is taken" answer is an account-enumeration oracle on an endpoint any signed-in user can call, Better Auth already answers the taken case with a silent `{ status: true }` and no mail (`update-user.mjs` L431-435), and uniqueness is enforced where it belongs, by the `UNIQUE` constraint on `"user".email`. The atomicity half the finding shares with F4 no longer depends on application code at all: the row that moves the identity is written inside Better Auth's `/api/auth/verify-email` handler, and `profiles.email` follows through the `sync_profile_email_on_user_update` trigger (`migrations/0008_sync_profile_email.sql`) in the same transaction, so the two rows cannot diverge - proved against the real database by `src/lib/__tests__/emailChangeVerification.db.test.ts`, whose fault-injection case shows the `"user"` row rolled back when the profile write fails. The UI's false promise ("A confirmation link will be sent to ...") is gone with the section, which is now the island `src/components/profile/EmailChangeSection.tsx`. Guarded by `src/lib/__tests__/identityWriteGuard.test.ts`, which fails on any `UPDATE "user" SET` or email-touching `UPDATE profiles SET` re-appearing under `src/`.
...
**Status:** Resolved by RH-66 (the commit carrying this line, on top of `a17dd2b`). The state pair is now the derived `const` the remediation asked for: `const currentUserId = session?.user?.id ?? null;` sits on the line directly under `const { data: session } = authClient.useSession();` (page L277), the `useState<string | null>` pair at L291 is deleted, and the copy at L343 went with it - taking `session?.user?.id` out of `refreshPlaylist`'s `useCallback` dependency list, which now reads `}, [playlistId, router, bandId]);` and would otherwise have left a `react-hooks/exhaustive-deps` "unnecessary dependency" warning behind; the `session` binding at L277 itself stays, because the derived `const` reads it. The value is still `string | null`, so the ownership gate on the playlist tag bar compares exactly what it compared before. The page's `complexity-budget/override` entry in `eslint.config.mjs` is re-pinned to the three freshly measured numbers, `35 / 1071 / 1343`, from `34 / 1072 / 1344`: the `complexity` ceiling went **up**, 34 -> 35, because ESLint counts `?.` and `??` alike and the coalescing moved out of `refreshPlaylist` (whose own complexity falls 6 -> 3) and into the component body - the file's total branch count falls, the outer function's own count rises by one - and `src/lib/__tests__/complexityBudget.test.ts` fails on any ceiling that is not exactly the file's current worst number, so leaving `34` in place would have failed the guard; RH-67 takes that number back down when the picker leaves the page, and the entry list is untouched (`grep -c "complexity-budget/override" eslint.config.mjs` prints `20` before and after, so AGENTS.md's "may only shrink" ratchet on the list is not loosened). The sweep the remediation asks for is done and comes back empty: `grep -rn "useState" src/hooks src/app src/components | grep -v __tests__ | grep -iE "session|userid|currentuser"` printed exactly one line at `a17dd2b` - this page's L291 - and prints nothing at all now, RH-64 (`6aa099c`) having already turned `useBandAdmin`'s copy into a derived `const currentUserId = session?.user?.id ?? null` at `src/hooks/useBandAdmin.ts:126-127`. The T8 close-out further down this same file (line 564 at `a17dd2b`, line 565 after this insertion) says F13 "stays open ... owned end to end by RH-53 (F11)": that sentence describes the state before this commit and is deliberately left standing as the record of RH-41's scope, so a later reader should not take it for F13's current state. The same commit lands `e2e/playlist-detail.spec.ts`, eleven serial Playwright tests driving the real page through opening a playlist, the catalog picker, removing a song, cycling a mastery status, song and playlist tags, the inline rename and both filters - all through accessible names the page already had, with no markup added - which is the characterization net that lets RH-67..RH-71 move this 1343-line component without rewriting it.
...
**Priority:** medium
**Covers:** F11, F20
**Status:** Delivered by RH-39 (`246313f`) for F20 and by RH-66 (`882806a`), RH-67 (`5c38206`), RH-68 (`acbbf83`), RH-69 (`a79abef`), RH-70 (`bd3c8cf`) and RH-71 (the commit carrying this line) for F11. Both findings it covers are now closed: `eslint.config.mjs` declares `complexity` 15, `max-depth` 4, `max-lines-per-function` 200, `max-params` 4 and `max-lines` 400 for everything under `src`, `src/lib/__tests__/complexityBudget.test.ts` enforces them because no CI job runs eslint, and the per-file override list is a ratchet that has shrunk to 19 entries, the playlist detail page's own entry having been deleted rather than re-homed.
...
**Priority:** high
**Covers:** F12
**Status:** Resolved by RH-42 (the commit carrying this line, on top of `890a27b`). Its only covered finding, F12, is closed above. The raw two-statement identity write is deleted rather than kept behind an admin check, and replaced by Better Auth's `changeEmail` flow: the verification link goes to the **new** address, an already-verified current address gets a confirmation first so the address losing the account is warned, and `profiles.email` follows the auth row through the `sync_profile_email_on_user_update` trigger instead of through application SQL. A uniqueness pre-check was deliberately omitted to avoid an account-enumeration oracle. Covered by `src/lib/__tests__/authConfig.test.ts`, `emailChange.test.ts`, `authEmail.test.ts`, `emailChangeVerification.db.test.ts`, `identityWriteGuard.test.ts` and `src/components/profile/__tests__/EmailChangeSection.test.tsx`.
