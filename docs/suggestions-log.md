...
## [RH-72] Fechar o CI E2E: fast-view-mobile nao encontra o heading (spec review r1) — 2026-09-10

- **The crash was already on file; the spec should close that loop.** The same
  `Object.defineProperty called on non-object` failure under `next dev --webpack`
  is recorded in `docs/suggestions-log.md` (RH-28 review, 2026-09-02, "A second,
  independent dev-only failure exists under the `--webpack` flag ... it does not
  occur under Turbopack") and deferred again by `docs/tasks/RH-19-spec.md:317-319`.
  The audit presents the diagnosis as new. Citing the prior entries — and marking
  them resolved, which the whitelist already permits — would keep the log honest.
- **Give the CI-on-production follow-up real numbers.** The Out of Scope bullet is
  correctly reasoned, but the entry it asks for in `docs/suggestions-log.md` would
  be far more useful carrying what I measured at `c8fffd1`: warm `npx next build`
  = 10.7s real; full suite against `npx next start` = `22 passed (19.9s)`, i.e.
  faster than the dev-server run; and `redirect param is honoured after login`
  still burning 10.8s in the retry loop there (the rate limiter, working as the
  spec predicts).
- **`--webpack` is not quite "the only webpack surface left".** `next.config.ts`
  still carries a `webpack(config, { isServer })` key (OpenTelemetry warning
  suppression) and `webpack@^5.106.2` is still a runtime dependency with a
  `knip.json` `ignoreDependencies` entry. After this change nothing runs either.
  ER6 rightly pins `next.config.ts`, so no action is needed here — but the new
  AGENTS.md decision entry could say the key survives only for ad-hoc
  `next build --webpack`, so the next reader does not think it is live.
- **A doc comment goes stale and the whitelist forbids fixing it.**
  `src/lib/pdfWorker.ts:8-9` says the copy-to-`public/` strategy "is what keeps it
  working identically under the Webpack dev server and the Turbopack build". Once
  no one runs webpack that sentence misleads. Either add that one file to the
  whitelist for a comment-only edit or log it.
- **ER8's `nextjs-agent-rules` pin contradicts AGENTS.md's own instruction.**
  `AGENTS.md:499` says the block is re-added by `next dev` and that "committing it
  with your work keeps the tree clean", while ER8 requires any regeneration be
  reverted. At Next 16.3.4 the regeneration is a no-op (the committed block was
  generated by this same version), so the pin is harmless in practice — but ER1
  requires running `npm run dev`, so the two instructions will be read side by side.
  Worth one clause reconciling them.
- **A pre-existing unit-test flake can fail ER7 for a correct implementation.**
  Running `rtk proxy npx vitest run` at `c8fffd1` I got
  `Test Files 1 failed | 106 passed (107)` / `Tests 1 failed | 1208 passed (1209)`:
  `src/components/profile/__tests__/EmailChangeSection.test.tsx > EmailChangeSection >
  renders the same copy for an address that already belongs to someone else`
  asserted the success banner while the component still rendered `Sending...`. The
  file passes 3/3 in isolation, and the `Coverage (vitest)` job was green on run
  34429486831, so this is a load-dependent race in the test, not a regression. ER7
  demands "exits 0 ... with no failed", so a re-run may be needed; worth logging as
  its own suggestion.
- **ER1's HMR step is the only manual, unpinned check in the set.** "Editing any
  visible English string in a client component under `src/components/`" is
  decidable but leaves the tester to pick the file. Naming one string (and
  requiring it be reverted, which ER8's closed set already enforces) would make it
  reproducible.

## Housekeeping

Everything I started was stopped: no process holds port 3000, and
`git status --short` shows only `?? docs/tasks/RH-72-spec.md`. Build/test byproducts
landed in gitignored paths (`.next/`, `test-results/`, `playwright-report/`); logs
were written to the session scratchpad, not the repo.

## [RH-72] Out-of-scope items recorded during implementation — 2026-09-10

- **Run the CI e2e job against a production build.** `.github/workflows/ci.yml`
  sets no `PLAYWRIGHT_WEB_SERVER`, so the `E2E Tests (Playwright)` job exercises
  `npm run dev`, never `next build` + `next start` — an RH-32-class production-only
  crash cannot be caught there. `playwright.config.ts` already supports the switch
  (`PLAYWRIGHT_WEB_SERVER="npx next start -p 3000 -H 127.0.0.1"`). The trade-off is
  real: a production server turns Better Auth's rate limiter on, which is exactly
  the flake RH-44 wrapped in `toPass` — measured at `c8fffd1` the production run was
  green and faster overall (`22 passed (19.9s)`) but `redirect param is honoured
  after login` spent ~10.8s inside the retry loop. Its own decision, not RH-72's.
- **Load `react-pdf` lazily on the Fast View route.** Deferring `TabDrawingStage`
  behind `next/dynamic` would keep ~1 MB of `pdfjs-dist` out of the route's first
  client payload — a real win for a page meant to open on stage over a phone
  connection. RH-72 deliberately did not do it: it would hide the webpack crash
  rather than remove it (Stage Mode would still die on any webpack dev build) and it
  touches `src/`.
- **`webpack` as a dependency, and its `knip.json` `ignoreDependencies` entry.**
  After RH-72 nothing in the repository runs webpack: `next.config.ts`'s
  `webpack(config, { isServer })` key (OpenTelemetry warning suppression) and the
  `webpack@^5.106.2` dependency survive only for an ad-hoc `next dev|build
  --webpack`. Whether both can be dropped is a dead-code question of its own.
- **`src/lib/pdfWorker.ts:8-9` now has a stale comment.** It says the
  copy-to-`public/` strategy "is what keeps it working identically under the Webpack
  dev server and the Turbopack build"; there is no Webpack dev server any more. A
  comment-only edit, outside RH-72's closed change set.
- **`pdfjs-dist` under Next 16's webpack dev runtime is an upstream bug.**
  `TypeError: Object.defineProperty called on non-object` thrown from
  `pdfjs-dist/build/pdf.mjs` via `__webpack_require__.r`. Not this repository's code;
  nobody has reported it upstream.

## [RH-72] Fechar o CI E2E: fast-view-mobile nao encontra o heading (code review r1) — 2026-09-10

1. **Log the pre-hydration controlled-input bug the deviation uncovered**
   (`docs/suggestions-log.md` is inside the permitted whitelist, so this can be
   added without opening the change set). The finding is not "Playwright is
   awkward": `src/app/login/page.tsx:116-125,140-149` are controlled inputs whose
   React state can never adopt text that reached the DOM before hydration, and
   `handleSubmit` posts state, so the form submits empty credentials while the
   fields visibly contain the values and `required` passes. Worth logging with the
   caveat I could not resolve statically — `LoginForm` is inside `<Suspense>` and
   uses `useSearchParams`, so a production prerender may deopt the boundary to
   client rendering and never ship the inputs in HTML, which would confine the
   window to dev; a production-build check would settle it. `/signup` and
   `/forgot-password` are the same shape and should be checked together. This is
   the kind of thing that is cheap to write down now and expensive to rediscover.

2. **Tighten `invalid credentials show an error message`** (`e2e/auth.spec.ts:106-110`).
   It asserts only that *some* error banner is visible and does not say "too many
   requests". Any 4xx from the sign-in route satisfies it — which is precisely how
   the empty-email bug could have kept that test green. Asserting the banner text
   matches the credential-rejection message (e.g. `/invalid email or password/i`)
   would make it fail loudly for the class of bug this task just spent a round
   diagnosing. Non-blocking, and arguably RH-44 territory.

3. **Consider whether the guard should also pin `build`.** `devBundler.test.ts`
   guards only the `dev` script, which is correct for the crash it names (the CI job
   runs `npm run dev`). But AGENTS.md now asserts "one bundler everywhere", and
   nothing mechanical stops `--webpack` appearing in `build`. One extra assertion on
   `scripts.build` would make the guard match the documented invariant. Genuinely
   optional — the spec scoped this to `dev` deliberately.

## [RH-72] Fechar o CI E2E: fast-view-mobile nao encontra o heading (QA r1) — 2026-09-10
