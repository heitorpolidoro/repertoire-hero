...
-- Description: Schema consolidado do Repertoire Hero.
--              Estado final limpo — sem dependências de GoTrue/auth.users.
--              Auth gerenciado pelo Better Auth (tabelas "user", "session",
--              "account", "verification" no schema public).
-- =============================================================================
...
-- ---------------------------------------------------------------------------
-- Table: global_songs
-- Catálogo colaborativo (wiki). contributor_id é informativo, não restritivo.
-- ---------------------------------------------------------------------------
...
COMMENT ON COLUMN global_songs.contributor_id IS 'Usuário que contribuiu a música. Pode atualizar, não implica posse exclusiva.';
...
DROP POLICY IF EXISTS "global_songs: authenticated users can update all rows" ON global_songs;
...
-- ---------------------------------------------------------------------------
-- Table: bands
-- Sem created_by — autoria é gerenciada por band_members.role = 'admin'.
-- ---------------------------------------------------------------------------
...
CREATE POLICY "profiles: band members can read co-member profiles"
    ...
    USING (id IN (
        SELECT user_id FROM band_members WHERE band_id = ANY(get_my_band_ids())
    ));
...
DROP POLICY IF EXISTS "bands: admins can delete" ON bands;
CREATE POLICY "bands: admins can delete" ON bands FOR DELETE
    USING (id IN (SELECT band_id FROM band_members WHERE user_id = auth.uid() AND role = 'admin'));
...
-- ---------------------------------------------------------------------------
-- RPCs para o fluxo de convite/criação de banda
-- p_user_id aceita chamadas server-side com admin client (sem auth.uid()).
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Table: repertoire
-- Coleção pessoal (user_id) ou de banda (band_id). Mutuamente exclusivos.
-- ---------------------------------------------------------------------------
